...
//****************************************************************************
//FUNCTION:
bool CScene::init(const std::string& vSerializedPath, const std::string& vBinFolderPath, bool vLegalTileName, unsigned int vBeginTileNumber, unsigned int vEndTileNumber, int vTraverseMaxDeep)
{
	...
	m_pItemTemplateCreator->generateAllTileItemTemplate(m_TileSet);
...
bool CScene::__deserializeFile(const std::string& vTilePath)
{
	...
	m_TileSet.emplace_back(TileRoot);
	return true;
}
...
std::shared_ptr<CTileNode> CScene::__parseRecord(const std::vector<NodeRecord>& vTreeRecord)
{
	...
	for (unsigned int i = 1; i < n; ++i)
	{
		...
		//m_TileNodeSet[TileNum][i].setParent(Result[vTreeRecord[i].second]);
		//m_TileNodeSet[TileNum][vTreeRecord[i].second].addChild(Result[i]);
	}
	return Result[0];
}
