...
namespace hivePagedLOD
{
	...
	public:
		...
		void addChild(const std::shared_ptr<CTileNode>& vChild) { _ASSERT(vChild); m_Children.push_back(vChild); }
		void clearChildren() { m_Children.clear(); }
		void addBrother(const std::shared_ptr<CTileNode>& vBrother) { _ASSERT(vBrother); m_Brother.push_back(vBrother); }
		...
		void setGeometryFileName(const std::string& vFileName) { _ASSERT(!vFileName.empty()); m_GeometryFileName = vFileName; }
		void setGeometryFileSize(std::uintmax_t vFileSize) { _ASSERT(vFileSize > 0); m_GeometryFileSize = vFileSize; }
		void setTextureFileName(const std::string& vFileName) { _ASSERT(!vFileName.empty()); m_TextureFileName = vFileName; }
		void setTextureFileSize(std::uintmax_t vFileSize) { _ASSERT(vFileSize > 0); m_TextureFileSize = vFileSize; }
		void setTriangleCount(std::uintmax_t vTriangleCount) { _ASSERT(vTriangleCount > 0); m_TriangleCount = vTriangleCount; }
		...
		void setAncestorUIDSet(const std::vector<unsigned int>& vSet) { _ASSERT(vSet.size() > 0); m_AncestorUIDSet = vSet; }
		void setBrotherUIDSet(const std::vector<unsigned int>& vSet) { _ASSERT(vSet.size() > 0); m_BrotherUIDSet = vSet; }
