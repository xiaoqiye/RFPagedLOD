...
TEST(TestLOD, generatePreferredTileNode) 
{
	...
	for (auto i = Size; i > 0; --i)
	{
		if (PreferredTileNodeGenerator.fetchOutputPipeline()->tryPop(PreferredTileNodeSet))
		{
			//Tile1 will prefer one TileNode
			EXPECT_TRUE(MockData::isSameTileNode(ExpectResultSet[i - 1], PreferredTileNodeSet[0][0]));
			//Tile2 is leaf node, it preferred anyway
			EXPECT_TRUE(MockData::isSameTileNode(TileNodeSet[TileNodeSet.size() - 1], PreferredTileNodeSet[1][0]));
		}
	}
}