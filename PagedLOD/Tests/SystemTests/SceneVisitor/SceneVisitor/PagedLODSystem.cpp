...
{
	m_pTileNodeLoaderThread->useRecord(true);
	...
	m_pRenderingTileNodeGeneratorThread->setLoadLimitSizePerSolve(vInitValue.LoadLimitPerSolve);
...
{
	//add
	const std::string AllKnapsackResultSavePath = "0426AllKnapsackResultSavePathRandom1.csv";
	const std::string AllKnapsackLoadInfoSavePath = "0426AllKnapsackLoadInfoSavePathRandom1.csv";

	const std::vector<std::vector<int>> TempAllKnapsackResult = m_pRenderingTileNodeGeneratorThread->getAllKnapsackResult();
	const std::vector<std::vector<double>> TempAllKnapsackLoadInfo = m_pTileNodeLoaderThread->getAllKnapsackLoadInfo();

	writeInfo2File(TempAllKnapsackResult, AllKnapsackResultSavePath);
	writeInfo2File(TempAllKnapsackLoadInfo, AllKnapsackLoadInfoSavePath);
...
//FUNCTION:
template<class Type>
void CPagedLODSystem::writeInfo2File(const std::vector<std::vector<Type>>& vInfo, const std::string& vFilePath)
{
	_ASSERT(!vFilePath.empty());

	std::ofstream OutputStream;
	OutputStream.open(vFilePath, std::ios::out | std::ios::app);
	if(!OutputStream.is_open())
		throw std::ios_base::failure("The file can't open!");

	for (unsigned int i = 0; i < vInfo.size(); ++i)
	{
		for (unsigned int k = 0; k < vInfo[i].size(); ++k)
		{
			if (k == 0)
			{
				OutputStream << vInfo[i][k];
				continue;
			}
			OutputStream << "," << vInfo[i][k];
		}
